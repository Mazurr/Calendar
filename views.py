...
class CalendarView(Frame):
    ...
    def __init__(self, window, **kwargs):
        ...
        self.y_button_left = Button(
            self, text="<", command=lambda: self.change_year(-1)
        )
        self.y_button_left.grid(row=0, column=0)
        self.y_button_right = Button(
            self, text=">", command=lambda: self.change_year(1)
        )
        ...
        self.m_button_left = Button(
            self, text="<", command=lambda: self.change_month(-1)
        )
        self.m_button_left.grid(row=1, column=0)
        self.m_button_right = Button(
            self, text=">", command=lambda: self.change_month(1)
        )
        self.m_button_right.grid(row=1, column=6)

        for i, w_day in enumerate(self.week):
    ...
    def render_days(self):
        ...
        for m_day in self.months[1]:
            month_b = Button(
                self.m_day_frame,
                text=m_day,
                command=lambda d=m_day: self.window.show_view(
                    "events_list_view", year=self.yr, month=self.month_n, day=d
                ),
                width=15,
                height=5,
                justify="center",
                bg=(today if m_day == self.dt.day else None),
            )
...
class AddEventsView(Frame):
    def __init__(self, window, **kwargs):
        ...
        cancel = Button(
            self,
            text="Cancel",
            command=lambda: window.show_view(
                "events_list_view", year=self.year, month=self.month, day=self.day
            ),
            width=50,
            height=2,
            justify="center",
...
class EventsListView(Frame):
    def __init__(self, window, **kwargs):
        ...
        for row in rows:
            ...
            r += 1
            del_ev = Button(
                self, text="Delete", command=lambda id=row[0]: self.delete_event(id=id)
            )
        ...
        add = Button(
            ...
            text="Add new event",
            command=lambda: window.show_view(
                "add_event_view", year=self.year, month=self.month, day=self.day
            ),
            width=50,
            height=2,
            justify="center",
        ...
        back = Button(
            ...
            text="Back",
            command=lambda: window.show_view(
                "calendar_view", year=self.year, month=self.month, day=self.day
            ),
            width=50,
            height=2,
            justify="center",
